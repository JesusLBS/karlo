...
class ProductRepositoryImpl extends ProductRepository {
    async index({ defaultOptions, where }) {
        ...
        const { count, rows } = await db.Product.scope("raw").findAndCountAll({
            where,
            include: [
                {
                  model: db.Business,
    ...
    countAll = async (where) => {
        return await db.Product.scope("raw").count({
            where,
            paranoid: false,
        });
    };
    ...
    async store(body, options = {}) {
        ...
        const row = await db.Product.create(body, { transaction });
        if (!row) return null;
        return row;
    }

    async edit(id) {
        const row = await db.Product.scope("raw").findOne({ where: { id } });
        if (!row) return null;
        return row;
    }
    ...
    async update(body, options = {}) {
        const transaction = options.transaction;
        const { id, ...rest } = body;
        const row = await db.Product.findOne({ where: { id }, transaction });
        if (!row) return null;
